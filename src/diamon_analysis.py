...
def filter_shutter_status(data, selected_shutter="all"):
    ...
    for result in data.values():
        ...
        if result.out_data.empty:
            continue
        #remove low current
        result = filter_low_beam_current(result)
        ...
        if "shutter-open" not in result.out_data.columns:
            ...
            filtered_list.append(convert_row_series(result, last_row))
            continue
        else:
            if selected_shutter == "closest":
                neighbours = result.beamlines.closest_neighbours.index
            else:
                neighbours = result.beamlines.all_neighbours.index
    ...
    df = pd.concat(filtered_list).set_index("key")
    return df
...
def drop_shutters(names, df):
    ...
    df["truth"] = shutter_keep[shutter_keep[names]].all(axis=1)
    return df
...
def filter_low_beam_current(data):
    """
    This function removes any data in out iles where beam current less than the argument minimum current
    Args:
        data (dict): dict of all data information
    ...
    if data.beamlines.target_station == "1":
        minimum_current = 120 # uAh
    elif data.beamlines.target_station == "2":
        minimum_current = 25
    data.out_data = data.out_data[data.out_data[data.beamlines.current_info] > minimum_current]
    return data
